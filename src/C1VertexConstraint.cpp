...
void C1VertexConstraint::apply(PolygonVertexItem *vertexItem) {
    ...
    if (castEdgeOut) applyC1ContinuityToOutgoingEdge(vertexItem, castEdgeOut);
    if (castEdgeIn)  applyC1ContinuityToIncomingEdge(vertexItem, castEdgeIn);
}
...
QPointF C1VertexConstraint::calculateControlPointPosition(const QPointF& basePos, const QPointF& targetPos, double fraction) {
