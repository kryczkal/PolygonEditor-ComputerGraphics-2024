...
BezierEdgeItem::BezierEdgeItem(PolygonVertexItem *start, PolygonVertexItem *end, QGraphicsItem *parent)
    : BaseEdgeItem(start, end, parent)
{
    Q_ASSERT(dynamic_cast<PolygonItem *>(parent) != nullptr); // Parent must be a PolygonItem

    QPointF startControlPointPos = start->edgeIn
                                       ? start->pos() + (start->pos() - start->edgeIn->getStartVertex()->pos())
                                       : (start->pos() + end->pos()) / 2;
    QPointF endControlPointPos   = end->edgeOut ? end->pos() + (end->pos() - end->edgeOut->getEndVertex()->pos())
                                                : (start->pos() + end->pos()) / 2;
...
BezierEdgeItem::BezierEdgeItem(BaseVertexItem *start, BaseVertexItem *end, QGraphicsItem *parent)
    : BaseEdgeItem(start, end, parent)
{
...
{
    int minX = std::min(
        {startVertex->pos().x(), endVertex->pos().x(), startControlPoint->pos().x(), endControlPoint->pos().x()}
    );
    int minY = std::min(
        {startVertex->pos().y(), endVertex->pos().y(), startControlPoint->pos().y(), endControlPoint->pos().y()}
    );
    int maxX = std::max(
        {startVertex->pos().x(), endVertex->pos().x(), startControlPoint->pos().x(), endControlPoint->pos().x()}
    );
...
BaseEdgeConstraint *BezierEdgeItem::getConstraint() const { return nullptr; }
