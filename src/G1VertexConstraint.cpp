...
void G1VertexConstraint::apply(PolygonVertexItem *vertexItem) {
    ...
    if (castEdgeOut) applyG1ContinuityToOutgoingEdge(vertexItem, castEdgeOut);
    if (castEdgeIn)  applyG1ContinuityToIncomingEdge(vertexItem, castEdgeIn);
}
...
void G1VertexConstraint::apply(ControlVertexItem *controlVertexItem) {
    ...
    if (controlVertexItem == castedParent->startControlPoint) apply(reinterpret_cast<PolygonVertexItem*>(castedParent->getStartVertex()));
    if (controlVertexItem == castedParent->endControlPoint) apply(reinterpret_cast<PolygonVertexItem*>(castedParent->getEndVertex()));
}
...
QPointF G1VertexConstraint::calculateParallelControlPoint(const QPointF& basePos, const QPointF& targetPos, const QPointF& currentControlPointPos) {
