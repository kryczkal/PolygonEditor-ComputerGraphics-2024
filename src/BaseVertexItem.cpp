...
QRectF BaseVertexItem::boundingRect() const { return QRectF(-radius, -radius, 2 * radius, 2 * radius); }

bool BaseVertexItem::hasBothEdges() const { return edgeOut != nullptr && edgeIn != nullptr; }

bool BaseVertexItem::hasOneEdge() const { return !hasBothEdges() && (edgeOut != nullptr || edgeIn != nullptr); }
...
void BaseVertexItem::mousePressEvent(QGraphicsSceneMouseEvent *event) { setFlag(QGraphicsItem::ItemIsMovable); }
